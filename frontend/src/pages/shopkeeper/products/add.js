    ...
    try {
      ...
      if (formData.image) {
        ...
        if (uploadResponse.data.success) {
          imageData = uploadResponse.data.data.image;
        } else {
      ...
      const payload = {
        ...
        category: formData.category, // must be ObjectId string from real categories
      ...
      if (response.status !== 201) throw new Error(response.data.message || 'Failed to add product');
